...
func (s *MemStorage) UpdateBatch(metrics []models.Metrics) ([]models.Metrics, error) {

	s.mu.Lock()

	defer s.mu.Unlock()
	updated := make([]models.Metrics, 0, len(metrics))
	for _, metric := range metrics {
		...
		case models.Gauge:
			...
			updated = append(updated, models.Metrics{
				ID:    metric.ID,
				MType: models.Gauge,
				Value: &value,
			})
